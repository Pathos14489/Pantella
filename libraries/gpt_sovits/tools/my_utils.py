    ...
    if sum(files_status)!=len(files_status):
        if is_train:
            for file,status in zip(file_list,files_status):
                if status:pass
                else:logging.warn(file)
            logging.warn('No Such File or Folder')
            return False
        elif is_dataset_processing:
            if files_status[0]:
                return True
            elif not files_status[0]:
                logging.warn(file_list[0])
            elif not files_status[1] and file_list[1]:
                logging.warn(file_list[1])
            logging.warn('No Such File or Folder')
            return False
        else:
            if file_list[0]:
                ...
                logging.warn('Expected No Empty Path')
            return False
    return True
...
def check_details(path_list=None,is_train=False,is_dataset_processing=False):
    if is_dataset_processing:
        ...
        if (not list_path.endswith('.list')):
            logging.warn('Please Fill in the Correct List Path')
            return
        if audio_path:
            if not os.path.isdir(audio_path):
                logging.warn('Please Fill in the Correct Audio Folder Path')
                return
    ...
    if is_train:
        ...
        with open(phone_path,'r',encoding='utf-8') as f:
            if f.read(1):...
            else:logging.warn('Missing Phoneme Dataset')
        if os.listdir(hubert_path):...
        else:logging.warn('Missing Hubert Dataset')
        if os.listdir(wav_path):...
        else:logging.warn('Missing Audio Dataset')
        df = pd.read_csv(
            semantic_path, delimiter="\t", encoding="utf-8"
        )
        if len(df) >= 1:...
        else:logging.warn('Missing Semantics Dataset')
