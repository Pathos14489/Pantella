...
class conversation_manager():
    ...
    def step(self): # process player input and NPC response until conversation ends at each step of the conversation
        ...
        if (self.character_manager.active_character_count() > 1) or (not radiant_dialogue):
            ...
            if (self.transcriber.activation_name_exists(transcript_cleaned, self.config.end_conversation_keyword.lower())) or (self.transcriber.activation_name_exists(transcript_cleaned, 'good bye')) or (self.transcriber.activation_name_exists(transcript_cleaned, 'goodbye')) or (conversation_ended.lower() == 'true'):
                self.game_state_manager.end_conversation(conversation_ended, self, self.tokenizer, self.synthesizer, self.chat_manager, self.messages, self.character_manager.active_characters, self.tokens_available, character)
            ...
            if transcribed_text: # If the player said something, add it to the messages
                if self.character_manager.active_character_count() > 1: # if multi NPC conversation
                    ...
                    perspective_player_name, perspective_player_desc, trust = self.character_manager.active_character.get_perspective_player_identity()
