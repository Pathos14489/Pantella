...
default = "xvasynth" # The default LLM to use if the one specified in config.ini is not found or if default is specified in config.ini
tts_Types = {}
...
def create_Synthesizer(conversation_manager):
    if conversation_manager.config.tts_engine not in tts_Types:
        logging.error(f"Could not find inference engine: {conversation_manager.config.tts_engine}! Please check your config.ini file and try again!")
