...
class base_Character:
    ...
    def gender(self):
        gender = "Male"
        if "gender" in self.info and self.info["gender"] != None and self.info["gender"] != "" and self.info["gender"].lower() != "either" and self.info["gender"].lower() != "neither":
    ...
    def get_perspective_identity(self, name, race, gender, relationship_level=0):
        """Get what the character perceives another character as"""
    ...
    def replacement_dict(self):
        """Return a dictionary of replacement strings for the character to use in the prompts -- Replaces in prompt placeholders with the character's information"""
    ...
    async def say(self,string, remember=True):
    ...
    async def leave_conversation(self):
        ...
        await self.say(random_goodbye+'.',False) # let the player know that the conversation is ending using the latest character in the conversation that isn't the player to say it
