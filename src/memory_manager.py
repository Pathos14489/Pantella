...
logging.info(f"Available memory managers: {Manager_Types.keys()}")
...
def create_manager(character_manager):
    ...
    logging.info(f"Creating memory manager for '{config.game_id}' using '{config.memory_manager}'...")
    if config.memory_manager != "auto": # if a specific memory manager is specified
        ...
        logging.info(f"Using memory manager {config.memory_manager}...")
        ...
        logging.info(f"Using memory manager {game_config['memory_manager']} recommended by game config...")
