...
class Tokenizer(tokenizer.base_Tokenizer): # Tokenizes(only availble for counting the tokens in a string presently for local_models), and parses and formats messages for use with the language model
    def __init__(self,conversation_manager, client):
        ...
        try:
            ...
            logging.error(f"Failed to load tiktoken encoding for model {self.config.openai_model}! Please check your {self.config.config_path} file and try again! If you're using a local model, try using the embedding tokenizer instead. It's slower, but more compatible.")
