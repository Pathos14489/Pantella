...
settings_description = {
    ...
    "gpt_sovits_device": "The device to run the GPT-SoVITS model on. This can be changed in your [game_id]_config.json. The default is 'cuda', but if you have a compatible NVIDIA GPU, you can set this to 'cuda' to significantly speed up synthesis times. If you don't have a compatible GPU, you can set this to 'cpu', but keep in mind that synthesis times will be significantly longer.",
    "gpt_sovits_version": "The version of the GPT-SoVITS model to use. This can be changed in your [game_id]_config.json. The default is 'v2', which is the latest version of the GPT-SoVITS model. The 'v1' version is the original version of the GPT-SoVITS model, which may have slightly lower audio quality but can be faster on older hardware.",
    "gpt_sovits_cut_type": "The method to use for cutting long texts. This can be changed in your [game_id]_config.json. The default is 'none', which means that no cutting will be done and the entire text will be synthesized at once. The 'cut1' method cuts the text into segments of 4 sentences, the 'cut2' method cuts the text into segments of approximately 50 characters, the 'cut3' method cuts the text by Chinese periods, the 'cut4' method cuts the text by English periods, and the 'cut5' method cuts the text by a variety of punctuation marks.",
    ...
    "gpt_sovits_banned_voice_models": "A list of voice models to ban from being used by GPT-SoVITS. This can be changed in your [game_id]_config.json. This is useful if you have a voice model that causes issues with GPT-SoVITS, such as extremely long synthesis times or crashes."
}
options = {
    "prompt_language": [
        {
